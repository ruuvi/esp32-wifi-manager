...
## Project overview
- This is **esp32-wifi-manager**, a modified fork of [tonyp7/esp32-wifi-manager](https://github.com/tonyp7/esp32-wifi-manager), extensively rewritten for integration with [Ruuvi Gateway](https://github.com/ruuvi/ruuvi.gateway_esp.c).
- It is an ESP-IDF component providing Wi-Fi network management via a built-in HTTP server and captive portal. Lightweight (~8 KB task stack), fully event-driven, can run entirely on PRO CPU.
- Built against **ESP-IDF v4.2.5**. Licensed under MIT.

## Scope and entry points
- Main runtime lives in `src/`; host-side unit tests live in `tests/`.
...
- Each test target compiles selected `src/*.c` directly plus wrapper libs (see `tests/test_http_req/CMakeLists.txt`).
- Coverage and allocator wrapping are standard in tests (`--coverage`, linker wraps for `malloc/calloc/free` in `tests/test_http_server_netconn_serve_handle_req/CMakeLists.txt`).
- Memory-leak assertions commonly use `os_malloc_trace_*` and log expectations (`tests/test_http_server_netconn_serve_handle_req/test_http_server_netconn_serve_handle_req.cpp`).
...
## Code style
- Code is formatted with **clang-format 14** using the `.clang-format` config in this directory (based on BARR-C:2018).
- To reformat all source and test files locally:
```bash
...
3. **`sonar-scan.yml` (SonarCloud Analysis)** — builds the `examples/default_demo` target under `build-wrapper-linux-x86-64`, then builds and runs tests with `--coverage`, generates `gcovr` coverage in SonarQube XML format, and uploads to SonarCloud (project key: `ruuvi_esp32-wifi-manager`, org: `ruuvi`).
...
- SonarCloud coverage uses `gcovr -r . --sonarqube -o coverage.xml` (not `lcov`).
